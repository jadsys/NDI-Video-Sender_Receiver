...
bool NdiCom::getWebcamFlg()
{
	...
	if (!strcasecmp(m_camera_types.c_str(), "web") == 0)
	{
		webcam_flg = false;
	}
	return (webcam_flg);
...
void NdiCom::sendCreate()
{
	...
	else if(strcasecmp(m_camera_types.c_str(), "ip") == 0)
	{
		// IPカメラの場合
		m_str_resname = string("IPCameraID:") + m_resources_id; // URL指定
	}
	else if (strcasecmp(m_camera_types.c_str(), "rs") == 0 || strcasecmp(m_camera_types.c_str(), "realsense") == 0)
	{
		// RealSenseカメラの場合
